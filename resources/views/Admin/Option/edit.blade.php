    ...
    <div class="card-body">
        <form method="POST" action="{{ Route('admin.options.update', $option->id) }}"
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control" id="catname" value="{{ $option->name }}">
            </div>
            <div id="input-container">
                <button id="add-input" class="btn btn-primary" type="button"> <i class="fa-solid fa-plus"></i> اضافة
                    خيار</button>
                @foreach($option->options as $key =>  $item)
                    <div class="mb-3 newselectss">
                        <div class="input-field">
                            <input class="form-control" type="text" name="options[]" id="option_{{$key}}"
                                placeholder="" value="{{$item}}">
                            <button class="remove-input btn btn-danger" type="button">Remove</button>
                        </div>
                    </div>
                @endforeach
            </div>
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            let counter = {{count($option->options)}};
